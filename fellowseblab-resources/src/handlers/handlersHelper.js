  ...
  for (var i = offset; i < offset + count; ++i) {
    if (i in integrationResponseItems) {
      ...
      ret.push({
        ...
        thumbnailHREF: integrationResponseItem.hasThumbnail
          ? `/books/${integrationResponseItem.resourceId}/thumbnail`
          : null,
      });
    }
  }
...
function buildResponseBody(endpoint, requestEvent, integrationResponse) {
  ...
  var countParam = requestEvent.queryStringParameters
    ...
    : null;
  var count = countParam ? parseInt(countParam, 10) : 9999;
  var offset = offsetParam ? parseInt(offsetParam, 10) : 0;
  var prevOffset;
  if (offset - count >= 0) {
