...
const batchWriteAddPutItems = (batchWriteInput, tableName, dataArr) => {
  if (dataArr && dataArr.length) {
    let newBatchWriteInput = batchWriteAddRequestItemsEntry(
      batchWriteInput,
      tableName,
    );
    newBatchWriteInput.RequestItems[tableName] = [
...
const batchWrite = async (dynamoDB, batchWriteInput) => {
  ...
  while (
    remaining &&
    remaining.RequestItems &&
    Object.getOwnPropertyNames(remaining.RequestItems).length
  ) {
...
class FellowsebLabDB {
  constructor({ isOffline, stage, resourcesTable, resourceTagsTable }) {
    ...
    this.resourceTagsTable =
      resourceTagsTable || `fellowseb-lab-${stage}-resourceTags`;
  }
  ...
  async batchWriteResources(resourcesToPut = []) {
    let batchWriteInput = batchWriteAddPutItems(
      {},
      this.resourcesTable,
      resourcesToPut,
    );
  ...
  async putResourceTags(resourceTags = []) {
    let resultArr = await Promise.all(
      resourceTags.map(async (resourceTag) => {
