...
const syncObject = async (key, sourceS3, sourceBucket, targetS3, targetBucket) => {
  const object = await sourceS3.getObject({ Bucket: sourceBucket, Key: key }).promise();
  if (object.ContentLength > 0) {
    process.stdout.write(`Putting object ${key}...\n`);
    return targetS3.putObject({
      Bucket: targetBucket,
      Key: key,
...
const main = async () => {
  ...
  try {
    ...
    for (let key of keys) {
      await syncObject(key, sourceS3, sourceOptions.bucket, sourceS3, targetOptions.bucket);
    }
  } catch (err) {
