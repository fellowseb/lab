...
class ResourceFeed extends React.PureComponent {
  ...
  render() {
    const { resources, onNextPage, onPrevPage } = this;
    const { error, loading, totalCount, filteredCount, next, prev } =
      this.state;
...
const updateComponentState = async (
  component,
  { filteredTag, count, apiUrl, apiEndpoint },
  offset,
) => {
  component.setState({ ...component.state, loading: true });
  try {
    let resources = await retrieveResources(
      apiUrl,
      apiEndpoint,
      filteredTag,
      count,
...
const retrieveResources = async (
  ...
  const paramString = (json) =>
    Object.keys(json)
      .reduce(
        (paramstr, key) =>
          json[key] ? paramstr.concat(key, "=", json[key], "&") : paramstr,
        "",
      )
      .slice(0, -1);
  const params = {
